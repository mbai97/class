...
if(Server::InitDataProvider())
{
    ...
    else
    {
        ...
        if(isset($_POST["form_userfile"]))
        {
            ...
            if(StoreScreenCapture($uId,$fInd,$fileId,$fileName))
            {
                exit("");
            }
        }
...
function StoreScreenCapture($_userId,$_fInd,&$_fId,&$_fName)
{
    $fileid = getId(32);
    ...
    if(IOStruct::CreateFile(PATH_UPLOADS . $filemask,base64_decode($_POST["form_userfile"]),false))
    {
...
function StoreFileChat($_userId,$_partner,$_fullname,$_chat)
{
    ...
    if(move_uploaded_file($_FILES["form_userfile"]["tmp_name"], PATH_UPLOADS . $filemask))
    {
        ...
        KnowledgeBase::CreateEntry($_partner, $fileid, $filemask, 4, $_FILES["form_userfile"]["name"], 0, $_userId, $_FILES["form_userfile"]["size"],"",$_chat->ChatId);
