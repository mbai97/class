...
class ServerPage
{
    ...
    static function GetDisabledFunctions()
    {
		...
		ob_start();
        phpinfo();
        $variable = strtolower(ob_get_contents());
        ob_get_clean();

        if(strpos($variable,"--disable-phar") !== false || strpos($variable,"phar") === false)
        ...
        if(!function_exists("mime_content_type"))
            $message .= "<span>Missing PHP extension: php_fileinfo. LiveZilla requires the PHP extension php_fileinfo in order to perform basic IO tasks. Please add the php_fileinfo to your PHP configuration (php.ini).</span><br><br>";
