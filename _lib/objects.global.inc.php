...
class Event extends BaseObject
{
	...
	function MatchVisitorConditions($_pageCount,$_timeOnSite,$_invAccepted,$_invDeclined,$_wasInChat,$_searchPhrase="",$_isMobile=false,$_country="",$_isInChat=false)
	{
		...
		if($this->PagesVisited > 0 && $_pageCount < $this->PagesVisited)
			$match = false;
		else if(!empty(Server::$Configuration->File["gl_vmac"]) && $this->TimeOnSite > 0 && $_timeOnSite < ($this->TimeOnSite-3))
        {
...
class KnowledgeBase
{
    ...
    static function CreateEntry($_userId, $_resId, $_value, $_type, $_title, $_disc, $_parentId, $_size=0, $_tags = "", $_chatId = 0)
    {
        if($_size == 0)
            $_size = strlen($_title);
        ...
        if(DBManager::GetRowCount($result) == 0)
        {
            if(!$_disc)
            {
                DBManager::Execute(true, "INSERT INTO `" . DB_PREFIX . DATABASE_RESOURCES . "` (`id`,`owner`,`editor`,`value`,`edited`,`title`,`created`,`type`,`discarded`,`parentid`,`size`,`tags`,`chat_id`,`related`) VALUES ('" . DBManager::RealEscape($_resId) . "','" . DBManager::RealEscape($_userId) . "','" . DBManager::RealEscape($_userId) . "','" . DBManager::RealEscape($_value) . "','" . DBManager::RealEscape(time()) . "','" . DBManager::RealEscape(Str::Cut($_title,248,false)) . "','" . DBManager::RealEscape(time()) . "','" . DBManager::RealEscape($_type) . "'," . intval(0) . ",'" . DBManager::RealEscape($_parentId) . "','" . DBManager::RealEscape($_size) . "','" . DBManager::RealEscape($_tags) . "'," . intval($_chatId) . ",'')");
            }
        }
        else
        {
    ...
    {
        DBManager::Execute(true, $d="UPDATE `" . DB_PREFIX . DATABASE_RESOURCES . "` SET `discarded`=1 WHERE `created`>(UNIX_TIMESTAMP()-86400) AND `parentid`=100 AND `type`=3 AND MD5(`id`)='". DBManager::RealEscape($_id) . "' LIMIT 1;");
    }
...
class KnowledgeBaseEntry
{
    ...
    function GetHTML($_color,$_inChat=true,$_lineBreak=true)
    {
        ...
        $html = str_replace("<!--link-->",KnowledgeBase::GetURL($this->Id,"",$this->Title,array(),false,true,$openExternal),$html);
...
class Cookie
{
    ...
    static function Get($_key,$_filter=false,$_maxlen=0)
    {
        ...
        $cd = base64_decode($_COOKIE["lz_" . $_key]);
        return $cd;
    }
